...
export async function consumeAiCreditAtomicInTransaction(
  ...
  if (
    !BILLABLE_AI_USAGE_TYPES.includes(
      type as (typeof BILLABLE_AI_USAGE_TYPES)[number],
    )
  ) {
  ...
  await tx.execute(
    sql`SELECT pg_advisory_xact_lock(hashtextextended(${"ai-credit:" + userId}, 0))`,
  );
  const [usage] = await tx
    ...
    .where(
      and(
        ...
        inArray(aiUsage.type, [...BILLABLE_AI_USAGE_TYPES]),
      ),
    );
  ...
  if (uses < allowedFreeCredits) {
    await tx
      .insert(aiUsage)
      .values({ userId, type: type as any, creditsUsed: 1, cost: 0 });
    ...
    return {
      consumed: true,
      source: "free",
      remainingPurchasedCredits: Number(wallet?.balance || 0),
    };
  }
  const [wallet] = await tx
    ...
    .where(
      and(eq(aiCreditWallets.userId, userId), gte(aiCreditWallets.balance, 1)),
    )
  ...
  if (wallet) {
    ...
    return {
      consumed: true,
      source: "purchased",
      remainingPurchasedCredits: wallet.balance,
    };
  }

  const walletPrice = parseLedgerAmount(walletPriceEGP, "AI wallet price");
  if (walletPrice <= 0)
    throw new Error("سعر استخدام الذكاء الاصطناعي غير صالح");
  await tx.execute(
    sql`SELECT pg_advisory_xact_lock(hashtextextended(${"wallet:" + userId}, 0))`,
  );
  ...
  await tx
    .insert(aiUsage)
    .values({ userId, type: type as any, creditsUsed: 1, cost: walletPrice });
  return {
    consumed: true,
    source: "wallet",
    remainingPurchasedCredits: 0,
...
export interface IStorage {
  ...
  createTransaction(
    tx: Omit<RevenueTransaction, "id" | "createdAt">,
  ): Promise<RevenueTransaction>;
  ...
  // File Uploads
  createUploadedFile(
    file: Omit<UploadedFile, "id" | "createdAt">,
  ...
  getPaymentServiceDeliveries(
    paymentRequestId: number,
  ): Promise<PaymentServiceDelivery[]>;
  ...
  approvePaymentRequestAtomic(
    ...
    transferReferenceRequired?: boolean;
    transferReferenceDuplicate?: boolean;
  }>;
  completePaymentServiceDeliveryAtomic(
    paymentRequestId: number,
    serviceType: string,
    adminUserId: string,
    deliveryNote?: string,
    ...
    payment: PaymentRequest | undefined;
    delivery: PaymentServiceDelivery | undefined;
    alreadyCompleted: boolean;
  }>;
  rejectPaymentRequestAtomic(
    id: number,
    adminNote?: string,
  ): Promise<{
    payment: PaymentRequest | undefined;
  ...
  updateTickerAd(
    id: number,
    data: Partial<TickerAd>,
  ): Promise<TickerAd | undefined>;
  deleteTickerAd(id: number): Promise<void>;
  deductTickerAdSecond(
    id: number,
    amountEGP: number,
  ): Promise<TickerAd | undefined>;
  deductTickerAdSecondAtomic(params: {
    adId: number;
    amountEGP: number;
    advertiserId: string;
    adminUserId: string;
  ...
  getUserSubscriptionInfo(
    userId: string,
  ): Promise<{ createdAt: Date; subscriptionEndsAt: Date | null }>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getAds(language?: string, userId?: string): Promise<Ad[]> {
    ...
    if (language) {
      return db
        .select()
        .from(ads)
        .where(eq(ads.language, language as any))
  ...
  async getAd(id: number): Promise<Ad | undefined> {
    ...
    if (ad)
      await db
        .update(ads)
        .set({ viewsCount: (ad.viewsCount || 0) + 1 })
        .where(eq(ads.id, id));
    return ad;
  }
  ...
  async createAdWithWalletDebitAtomic(
    ...
    if (amount <= 0)
      throw new Error(
        "createAdWithWalletDebitAtomic: المبلغ يجب أن يكون أكبر من صفر",
      );
    return await db.transaction(async (tx) => {
      await tx.execute(
        sql`SELECT pg_advisory_xact_lock(hashtextextended(${"wallet:" + insertAd.userId}, 0))`,
      );
      const [row] = await tx
        .select({
          balance: sql<string>`COALESCE(SUM(
      ...
      await tx.execute(
        sql`UPDATE ads SET expires_at = ${expiresAt} WHERE id = ${ad.id}`,
      );
      ...
      return db
        ...
        .where(
          and(
            ...
            eq(channels.language, language as any),
          ),
        )
  ...
  async getLiveStreams(status?: string): Promise<LiveStream[]> {
    ...
    return db
      .select()
      .from(liveStreams)
      .where(eq(liveStreams.status, st as any))
    ...
    return db
      ...
      .where(
        and(
          ...
          eq(chatMessages.isHidden, false),
        ),
      )
    ...
    const [like] = await db
      ...
      .where(
        and(
          ...
          eq(likes.targetType, targetType as any),
  ...
  async toggleLike(
    ...
    if (existing) {
      ...
      if (targetType === "ad")
        await db
          .update(ads)
          .set({ likesCount: sql`GREATEST(0, ${ads.likesCount} - 1)` })
          .where(eq(ads.id, targetId));
      if (targetType === "stream")
        await db
          .update(liveStreams)
          .set({ likesCount: sql`GREATEST(0, ${liveStreams.likesCount} - 1)` })
          .where(eq(liveStreams.id, targetId));
      if (targetType === "reel")
        await db
          .update(reels)
          .set({ likesCount: sql`GREATEST(0, ${reels.likesCount} - 1)` })
          .where(eq(reels.id, targetId));
      return { liked: false };
    } else {
      await db
        .insert(likes)
        .values({ userId, targetType: targetType as any, targetId });
      if (targetType === "ad")
        await db
          .update(ads)
          .set({ likesCount: sql`${ads.likesCount} + 1` })
          .where(eq(ads.id, targetId));
      if (targetType === "stream")
        await db
          .update(liveStreams)
          .set({ likesCount: sql`${liveStreams.likesCount} + 1` })
          .where(eq(liveStreams.id, targetId));
      if (targetType === "reel")
        await db
          .update(reels)
  ...
  async createComment(comment: InsertComment): Promise<Comment> {
    ...
    if (comment.targetType === "ad") {
      await db
        .update(ads)
        .set({ commentsCount: sql`${ads.commentsCount} + 1` })
        .where(eq(ads.id, comment.targetId));
    }
    if (comment.targetType === "reel") {
      await db
        .update(reels)
        .set({ commentsCount: sql`${reels.commentsCount} + 1` })
        .where(eq(reels.id, comment.targetId));
    }
    return c;
  }
    ...
    const [f] = await db
      ...
      .where(
        and(
          ...
          eq(follows.channelId, channelId),
        ),
      );
    return f;
  }
  ...
  async createAdCampaign(campaign: InsertAdCampaign): Promise<AdCampaign> {
    ...
    const [updated] = await db
      .update(adCampaigns)
      .set({ embedCode, clickTrackingCode: clickCode })
  ...
  private async getPlatformRates(): Promise<{
    cpmRate: number;
    cpcRate: number;
    pubPct: number;
  }> {
  ...
  async recordImpression(
    ...
    return await db.transaction(async (tx) => {
      ...
      await tx.execute(
        sql`SELECT pg_advisory_xact_lock(hashtextextended(${"wallet:" + locked.advertiserId}, 0))`,
      );
      ...
      const [wallet] = await tx
        ...
        .from(revenueTransactions)
        .where(eq(revenueTransactions.userId, locked.advertiserId));
      if (
        parseLedgerAmount(wallet?.balance ?? 0, "wallet balance") < revenueEGP
      ) {
        await tx
          .update(adCampaigns)
          .set({ status: "paused" })
      ...
      if (budget > 0) {
        ...
        if (ratio >= 0.8) {
          return {
            budgetWarning: true,
            budgetRatio: ratio,
            advertiserId: locked.advertiserId,
            campaignName: locked.name,
          };
        }
      }
      return {};
  ...
  async getRevenueTransactions(
    ...
    if (options.channelId !== undefined)
      conds.push(eq(revenueTransactions.channelId, options.channelId));
    if (options.from)
      conds.push(gte(revenueTransactions.createdAt, options.from));
  ...
  async getRevenueTotals(
    ...
    return {
      ...
      wallet_recharge: parseLedgerAmount(
        row?.wallet_recharge ?? 0,
  ...
  async getWithdrawableBalanceEGP(userId: string): Promise<number> {
    ...
    return Math.max(
      0,
      parseLedgerAmount(row?.balance ?? 0, "withdrawable balance"),
    );
  }
  ...
  async updateReport(
    ...
    const [r] = await db
      .update(reports)
      .set({ status: status as any, adminNote })
  ...
  async getPaymentServiceDeliveries(
    paymentRequestId: number,
  ): Promise<PaymentServiceDelivery[]> {
    return db
      .select()
  ...
  async approvePaymentRequestAtomic(
    ...
    return await db.transaction(async (tx) => {
      ...
      if (locked.type === "withdrawal") {
        canonicalTransferReference =
          canonicalPaymentReference(transferReference);
        if (
          !canonicalTransferReference ||
          transferReference!.trim().length > 160
        ) {
          return {
            payment: locked,
            alreadyProcessed: false,
            transferReferenceRequired: true,
          };
        }
        await tx.execute(
          sql`SELECT pg_advisory_xact_lock(hashtextextended(${outgoingTransferReferenceLockKey(canonicalTransferReference)}, 0))`,
        );
        ...
        await tx.execute(
          sql`SELECT pg_advisory_xact_lock(hashtextextended(${"wallet:" + locked.userId}, 0))`,
        );
        ...
        const currentBalanceEGP = parseLedgerAmount(
          balRow?.balance ?? 0,
          "wallet balance",
        );
        const requested = parseLedgerAmount(
          locked.amountEGP ?? 0,
          "withdrawal amount",
        );
        ...
        if (legacyReferenceUse.rows.length > 0) {
          return {
            payment: locked,
            alreadyProcessed: false,
            transferReferenceDuplicate: true,
          };
        }
      ...
      if (locked.type === "withdrawal") {
        ...
        if (
          ...
          throw new Error(
            "الكمية الأكبر من واحد متاحة فقط لرصيد الذكاء الاصطناعي أو الكمية المحسوبة لباقة العملات",
          );
        }
        ...
        if (isWalletRecharge) {
          ...
          for (const service of services) {
            ...
            if (service === "ad_boost") {
              if (!locked.adId)
                throw new Error("طلب تعزيز الإعلان غير مرتبط بإعلان");
              ...
              `);
              if (result.rows.length === 0)
                throw new Error("تعذر العثور على الإعلان المطلوب تجديده");
              ...
              if (!locked.servicePackageId)
                throw new Error("طلب باقة العملات لا يحدد الباقة");
              ...
              if (wallet.rows.length !== 1)
                throw new Error("تعذر شحن محفظة العملات");
      ...
      const [updated] = await tx
        ...
        .set({
          ...
          fulfilledAt,
          transferReference:
            locked.type === "withdrawal" ? transferReference!.trim() : null,
  ...
  async completePaymentServiceDeliveryAtomic(
    ...
    return db.transaction(async (tx) => {
      ...
      const [delivery] = await tx
        ...
        .for("update");
      if (!delivery)
        return { payment, delivery: undefined, alreadyCompleted: false };
      if (delivery.status === "completed")
        return { payment, delivery, alreadyCompleted: true };
      const [completed] = await tx
        .update(paymentServiceDeliveries)
        .set({
          status: "completed",
        ...
        .where(eq(paymentServiceDeliveries.id, delivery.id))
        .returning();
      const rows = await tx
        .select({ status: paymentServiceDeliveries.status })
        .from(paymentServiceDeliveries)
        .where(eq(paymentServiceDeliveries.paymentRequestId, paymentRequestId));
   ...
   */
  async rejectPaymentRequestAtomic(
    id: number,
    adminNote?: string,
  ): Promise<{
    payment: PaymentRequest | undefined;
  ...
  async deductTickerAdSecondAtomic(params: {
    ...
    return await db.transaction(async (tx) => {
      ...
      if (chargeAdvertiser && amountEGP > 0) {
        await tx.execute(
          sql`SELECT pg_advisory_xact_lock(hashtextextended(${"wallet:" + advertiserId}, 0))`,
        );
        const [balanceRow] = await tx
          .select({
            balance: sql<string>`COALESCE(SUM(
        ...
        if (
          parseLedgerAmount(balanceRow?.balance ?? 0, "wallet balance") <
          amountEGP
        )
          return undefined;
      }
  ...
  async getStats(): Promise<any> {
    ...
    const [totalImpressions] = await db
      .select({ total: sql<number>`sum(impressions)` })
      .from(adCampaigns);
    const [totalRevenueEGP] = await db
      .select({ total: sql<number>`sum(spent_egp)` })
      .from(adCampaigns);
  ...
  async updateUserSubscription(userId: string, endsAt: Date): Promise<void> {
    await db.execute(
      sql`UPDATE users SET subscription_ends_at = ${endsAt} WHERE id = ${userId}`,
    );
  }
  ...
  async getUserSubscriptionInfo(
    ...
    const rows = await db.execute(
      sql`SELECT created_at, subscription_ends_at FROM users WHERE id = ${userId} LIMIT 1`,
    );
